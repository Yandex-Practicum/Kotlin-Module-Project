abstract class CommonMenu(private val typeElement: TitleTypes, private val archName : String?){
    ...
    //метод onCreate() - общий для окна "Архивы" и для "Заметки"
    //в нем - отображение пунктов меню
    fun onCreate(){
        NoteTools.printArchiveNotesHead(typeElement)
        if(typeElement.name == "NOTES" && archName!=null) {
            println("Работа с архивом: $archName")
        }
